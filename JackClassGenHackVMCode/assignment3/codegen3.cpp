    ...
    if(control.tk.process=="walk_subr_call"){
        ...
        print_status("walk_var:control.stack_status:walk_let");
            ...
            if(node.name!="" && LState.func){
                ...
                if(scontrol.contain({"walk_function","walk_subr_body","walk_statements","walk_subr_call:control.walk_subr_call_args","walk_statement","walk_let","walk_subr_call:control.walk_subr_call_args","walk_var"})){
            
                }
                else if(SymbolControl.find_node_list(name,"function","local","").size()>0){
...
void walk_subr_call(ast t)
{
    ...
    print_status(34.1,"walk_subr_call:control.walk_subr_call_args");
    int call_args=0;
